...
static void *string_alloc_cb(struct theft *t, theft_hash seed, void *env) {
  ...
  char *str = static_cast<char *>(malloc(sz + 1));
  if (str == NULL) {
    return THEFT_ERROR;
  }
...
/*   theft_free(t); */
/*   GREATEST_ASSERT_EQm("should_return_results_if_there_is_a_match", */
/*                       THEFT_RUN_PASS, res); */
/*   PASS(); */
/* } */

/* static theft_trial_res */
/* prop_positions_should_match_characters_in_string(char *needle, char
 * *haystack) { */
/*   int match_exists = has_match(needle, haystack); */
/*   if (!match_exists) */
/*     return THEFT_TRIAL_SKIP; */
...
/*   /1* Matching characters must be in returned positions *1/ */
/*   for (int i = 0; i < n; i++) { */
